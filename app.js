...
async function refreshDashboard(){
 ...
 const kd=items.filter(i=>i.module==="kondo"&&i.category!=="__META__"),kp=kd.length?Math.round(kd.filter(i=>i.completed).length/kd.length*100):0;
 ...
 dashboard.innerHTML=`<div class="dash-head"><div><small>PERSONAL COMMAND CENTER</small><h1>Dashboard</h1><p>${new Date().toLocaleDateString(undefined,{weekday:"long",month:"long",day:"numeric"})}</p></div><button id="dQuick">＋ Quick Add</button></div>
 <div class="dash-stats"><button data-m="tasks"><b>${openTasks.length}</b><span>Open Tasks</span></button><button data-m="projects"><b>${activeProjects}</b><span>Projects</span></button><button data-m="travel"><b>${trips}</b><span>Trips</span></button><button data-m="shopping"><b>${groceries}</b><span>Groceries</span></button></div>
 ...
 dashboard.querySelectorAll("[data-m]").forEach(b=>b.onclick=()=>openModule(b.dataset.m));document.getElementById("dQuick").onclick=()=>quickAddBtn.click()
}
