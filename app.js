...
const modules=[{key:"finance",name:"Net Worth",icon:"💰"},{key:"planes",name:"Planes Near Me",icon:"✈️"},{key:"travel",name:"Travel",icon:"🌎"},{key:"houses",name:"Houses",icon:"🏡"},{key:"shopping",name:"Groceries",icon:"🛒"},{key:"recipes",name:"Recipes",icon:"🍳"},{key:"prints",name:"3D Print List",icon:"🖨️"},{key:"shortGoals",name:"Short-Term Goals",icon:"🎯"},{key:"longGoals",name:"Long-Term Goals",icon:"🏆"},{key:"projects",name:"Projects",icon:"🔨"},{key:"tasks",name:"Tasks",icon:"✅"}];
...
return common}
function openItemForm(item=null){const isEdit=!!item;showSheet(isEdit?"Edit item":"Add item",moduleSpecificFields(currentModule,item||{})+`<div class="form-actions"><button class="ghost-btn" id="cancelForm">Cancel</button><button class="primary-btn" id="saveForm">${isEdit?"Save changes":"Add"}</button></div>`);cancelForm.onclick=hideSheet;saveForm.onclick=async()=>{const obj={...(item||{}),module:currentModule,title:fTitle.value.trim(),details:fDetails.value.trim(),updatedAt:Date.now()};if(!obj.title)return alert("Add a title.");for(const [id,key] of [["fStore","store"],["fDate","date"],["fPriority","priority"],["fAmount","amount"],["fRating","rating"],["fUrl","url"],["fStatus","status"],["fDestination","destination"],["fDepartureDate","departureDate"],["fReturnDate","returnDate"],["fFlights","flights"],["fHotel","hotel"],["fConfirmations","confirmations"],["fTransportation","transportation"],["fActivities","activities"],["fStillToBook","stillToBook"],["fQuantity","quantity"],["fCategory","category"],["fRecipeUrl","recipeUrl"],["fServings","servings"],["fPrepTime","prepTime"],["fCookTime","cookTime"],["fIngredients","ingredients"],["fInstructions","instructions"]]){const el=document.getElementById(id);if(el)obj[key]=el.value}const comp=document.getElementById("fCompleted");if(comp)obj.completed=comp.checked;
...
if(isEdit)await putTo("items",obj);else await addTo("items",obj);hideSheet();await renderModule();await refreshDashboard()}}
